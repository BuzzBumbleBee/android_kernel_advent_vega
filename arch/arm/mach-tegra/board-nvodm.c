...
#include <linux/i2c.h> // Ant
#include <linux/irq.h> // Ant
#include <linux/delay.h> // Ant
...
static struct tegra_sdhci_platform_data tegra_sdhci_platform[] = {
	...
	[3] = {
		.bus_width = 4,
		.debounce = 3000,	//Kenji suggest for SD card detection, Ant mod
		//.debounce = 5,
	},
};
...
void __init tegra_setup_nvodm(bool standard_i2c, bool standard_spi)
{
	...
	i2c_register_board_info(3, tp_i2c_device1,
		ARRAY_SIZE(tp_i2c_device1));
	i2c_register_board_info(3, tp_i2c_device2,
		ARRAY_SIZE(tp_i2c_device2));
	// 3G power on set GPIO_PJ7 output low
	gpio_request(79, "3G power") ;
   // >>> [101221] manual set gpio to high [Derek]
	gpio_direction_output(79, 0) ;
   // <<< [101221] manual set gpio to high [Derek]

	// GPS -- power off set GPIO_PV0 output high
	//gpio_request(168, "gps power") ;
	//gpio_direction_output(168, 0) ;
	//// Recovery notify gpio -- set GPIO_PB0 output high
	//gpio_request(8, "recovery") ;
