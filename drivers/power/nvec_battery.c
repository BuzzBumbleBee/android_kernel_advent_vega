...
static NvBool tegra_battery_data(NvOdmBatteryInstance NvBatteryInst)
{
	...
	// Ant start
	//printk(KERN_ERR "Ant true tegra_battery_data batt_dev->voltage: %d\n", batt_dev->voltage);
	//printk(KERN_ERR "Ant true tegra_battery_data batt_dev->current_ma: %d\n", batt_dev->current_ma);
	//printk(KERN_ERR "Ant true tegra_battery_data batt_dev->current_avg: %d\n", batt_dev->current_avg);
	//printk(KERN_ERR "Ant true tegra_battery_data batt_dev->temp: %d\n", batt_dev->temp);
	//printk(KERN_ERR "Ant true tegra_battery_data batt_dev->percent_remain: %d\n", batt_dev->percent_remain);
	//printk(KERN_ERR "Ant true tegra_battery_data batt_dev->lifetime: %d\n", batt_dev->lifetime);
	//printk(KERN_ERR "Ant true tegra_battery_data batt_dev->consumed: %d\n", batt_dev->consumed);
	//printk(KERN_ERR "Ant true tegra_battery_data batt_dev->capacity: %d\n", batt_dev->capacity);
	//printk(KERN_ERR "Ant true tegra_battery_data batt_dev->capacity_crit: %d\n", batt_dev->capacity_crit);
	//printk(KERN_ERR "Ant true tegra_battery_data batt_dev->capacity_remain: %d\n", batt_dev->capacity_remain);
	// Ant end
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		if (!batt_dev->present ) {
			...
			// Ant start
			printk(KERN_ERR "Ant batt is not present\n");
			// Ant end
		}
		else {
			/*
...
int nvec_battery_start(void)
{
	NvOdmBatteryDeviceOpen(&(batt_dev->hOdmBattDev),
			(NvOdmOsSemaphoreHandle *)&batt_dev->hOdmSemaphore);
	setup_timer(&batt_dev->battery_poll_timer, tegra_battery_poll_timer_func, 0);
	mod_timer(&batt_dev->battery_poll_timer,
			jiffies + msecs_to_jiffies(batt_dev->batt_status_poll_period));
...
int __init nvec_battery_init(void)
{
	int err;
...
void __exit nvec_battery_exit(void)
{
...
EXPORT_SYMBOL(nvec_battery_stop);
EXPORT_SYMBOL(nvec_battery_start);
