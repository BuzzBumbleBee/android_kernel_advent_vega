...
//Kenji+>
/*
static void card_detection_event(struct work_struct *work)
{
	...
	printk(KERN_INFO "SD Card detected 1, Card present=%d.\n",
		host->card_present);
...
*/
static void card_detect_debounce_timer(void *dev_id)
{
...
static irqreturn_t card_detect_isr1(int irq, void *dev_id)
{
	struct sdhci_host *sdhost = dev_id;
	struct tegra_sdhci *host = sdhci_priv(sdhost);

	BUG_ON(irq != gpio_to_irq(host->gpio_cd));
	
	// Ant start
	if(host->debounce == 3000)
	// Ant end
	{
		mod_timer(&host->timer,
		jiffies + msecs_to_jiffies(host->debounce));
	}
...
int __init tegra_sdhci_probe(struct platform_device *pdev)
{
	...
	if (plat->max_clk)
		...
		//dev_info(&pdev->dev, "Ant no max_clk specified, default to 25MHz\n");
	...
	if (host->gpio_cd != -1) {
		...
		//Kenji+>
		setup_timer(&host->timer, card_detect_debounce_timer, (unsigned long)sdhost);
		//INIT_WORK(&host->work, card_detection_event);
		ret = request_irq(host->irq_cd, card_detect_isr1,
